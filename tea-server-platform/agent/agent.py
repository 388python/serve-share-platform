...
ALLOWED_NAME_PATTERN = re.compile(r'^[a-zA-Z0-9][a-zA-Z0-9_-]*$')
...
def validate_name(name):
    ...
    if len(name) > MAX_NAME_LENGTH:
        raise ValueError(f"名称长度不能超过 {MAX_NAME_LENGTH} 字符")
    if not ALLOWED_NAME_PATTERN.match(name):
        raise ValueError("名称只能包含字母、数字、下划线和连字符，且必须以字母或数字开头")
    return name
...
def validate_command(command):
    ...
    if len(command) > MAX_COMMAND_LENGTH:
        raise ValueError(f"命令长度不能超过 {MAX_COMMAND_LENGTH} 字符")
    # 检查危险字符和命令
    dangerous_patterns = [
        r'\$\(', r'\$\{', r'\`',  # 命令替换
        r';\s*rm', r';\s*dd', r';\s*mkfs',  # 危险命令
        r'\|\s*rm', r'\|\s*dd',  # 管道危险命令
        r'>\s*/dev/', r'>\s*/etc/',  # 重定向到敏感位置
        r'eval\s', r'exec\s',  # 动态执行
    ]
    for pattern in dangerous_patterns:
        if re.search(pattern, command, re.IGNORECASE):
            raise ValueError(f"命令包含不允许的操作")
    return command
...
def safe_subprocess_run(cmd_args, timeout=DEFAULT_TIMEOUT, **kwargs):
    """安全执行子进程命令"""
    try:
        ...
        logger.error(f"命令执行超时: {' '.join(cmd_args[:3])}...")
        raise
    except subprocess.SubprocessError as e:
        logger.error(f"子进程执行错误: {e}")
        raise
...
# 验证API密钥最小长度
if len(API_KEY) < 16:
    raise SystemExit("AGENT_API_KEY must be at least 16 characters for security")
    ...
    for port in range(PORT_RANGE_START, PORT_RANGE_END):
        if port not in USED_PORTS:
            ...
            if f":{port}" not in result.stdout:
                USED_PORTS.add(port)
                logger.debug(f"Port {port} allocated")
                return port
...
def ensure_dependencies():
    ...
    for cmd, pkg in deps:
        result = safe_subprocess_run(["which", cmd], timeout=5)
    ...
    result = safe_subprocess_run(["which", "virt-builder"], timeout=5)
    if result.returncode != 0:
    ...
    # 确保 libvirtd 运行
    safe_subprocess_run(["systemctl", "start", "libvirtd"], timeout=10)
    safe_subprocess_run(["systemctl", "enable", "libvirtd"], timeout=10)
...
def setup_port_forwarding(host_port, vm_ip, vm_port, protocol="tcp"):
    """设置端口转发规则"""
    # 验证IP地址格式
    if not re.match(r'^\d+\.\d+\.\d+\.\d+$', vm_ip):
        logger.warning(f"Invalid IP address for port forwarding: {vm_ip}")
        return False
    ...
    logger.info(f"Port forwarding: {host_port} -> {vm_ip}:{vm_port}")
    return True
...
def remove_port_forwarding(host_port, vm_ip, vm_port, protocol="tcp"):
    ...
    # 验证IP地址格式
    if not re.match(r'^\d+\.\d+\.\d+\.\d+$', vm_ip):
        logger.warning(f"Invalid IP address for port forwarding removal: {vm_ip}")
        return False
    ...
    logger.info(f"Removed port forwarding: {host_port} -> {vm_ip}:{vm_port}")
    return True

def _lxc_exec(name, cmd, timeout=DEFAULT_TIMEOUT):
    """Execute command inside LXD container safely"""
    validate_name(name)
...
def _lxc_exec_safe(name, cmd_args, timeout=DEFAULT_TIMEOUT):
    """Execute command inside LXD container with safe argument passing"""
    validate_name(name)
    return safe_subprocess_run(
        ["lxc", "exec", name, "--"] + cmd_args,
        timeout=timeout
    )

def _inject_ssh_key(name, public_key, virt="lxd"):
    """Inject SSH public key and enable SSH service safely"""
    if not public_key:
        return

    # 验证SSH公钥格式（防止注入）
    ssh_key_pattern = re.compile(r'^ssh-(rsa|ed25519|dss|ecdsa-sha2-nistp(?:256|384|521))\s+[A-Za-z0-9+/=]+(?:\s+.*)?$')
    if not ssh_key_pattern.match(public_key.strip()):
        logger.warning(f"Invalid SSH key format for {name}")
        return
    ...
    if virt == "lxd":
        ...
        try:
            ...
            # Append public key to authorized_keys - 使用安全的方式传递
            # 使用printf而不是echo来避免特殊字符问题
            safe_key = escape_shell_arg(public_key)
            _lxc_exec(name, f"printf '%s\\n' {safe_key} >> /root/.ssh/authorized_keys", timeout=10)
            ...
            logger.info(f"SSH key injected for {name}")
        except Exception as e:
            logger.error(f"Failed to inject SSH key for {name}: {e}")

class AgentHandler(BaseHTTPRequestHandler):
    def _check_auth(self):
        """安全验证API密钥（使用timing-safe比较防止时序攻击）"""
        api_key = self.headers.get("X-API-Key", "")
        if not api_key:
            return False
        # 使用timing-safe比较防止时序攻击
        return hashlib.sha256(api_key.encode()).hexdigest() == hashlib.sha256(API_KEY.encode()).hexdigest()
    ...
    def do_GET(self):
        if not self._check_auth():
            ...
            logger.warning("Unauthorized GET request")
            return
        ...
        try:
            ...
            logger.warning(f"Validation error in GET {path}: {e}")
            self._send_json({"error": str(e)}, 400)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout in GET {path}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
        ...
        try:
            ...
            if virt == "lxd":
                ...
                if result.returncode == 0:
                    ...
                    logger.error(f"lxc list failed for {name}: {result.stderr}")
                ...
                if result.returncode == 0:
                    ...
                    # Get VNC port
                    vnc_result = safe_subprocess_run(
                    ...
                    novnc_result = safe_subprocess_run(
                    ...
                    blk_result = safe_subprocess_run(
                        ["virsh", "domblklist", name, "--details"],
                        timeout=10
                    )
                    image = ""
                    if blk_result.returncode == 0:
                        lines = blk_result.stdout.split("\n")
                        if len(lines) > 2:
                            image = os.path.basename(lines[2].split()[-1] if lines[2].split() else "")
                    ...
                    logger.error(f"virsh dominfo failed for {name}: {result.stderr}")
                    self._send_json({"error": "machine not found"}, 404)
        except ValueError as e:
            logger.warning(f"Invalid name for machine info: {e}")
            self._send_json({"error": str(e)}, 400)
        except json.JSONDecodeError as e:
            logger.error(f"JSON decode error for machine {name}: {e}")
            self._send_json({"error": "invalid response format"}, 500)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout getting info for machine {name}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
            logger.error(f"Error getting machine info for {name}: {e}")
            self._send_json({"error": str(e)}, 500)
    ...
    def _setup_novnc(self, name, vnc_port):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"noVNC started for {name} on port {web_port}")
                return web_port
            else:
                logger.warning(f"Failed to start novnc for {name}: {result.stderr}")
                return 0
        except Exception as e:
            logger.warning(f"novnc setup failed for {name}: {e}")
            return 0
    ...
    def _prepare_windows_image(self, image, kvm_base):
        ...
        if os.path.exists(kvm_base):
            logger.info(f"Windows base image already exists: {kvm_base}")
            return True
        ...
        if not download_url:
            logger.info(f"No download URL for {image}, trying virt-builder")
            return self._build_windows_with_virtbuilder(image, kvm_base)

        # 尝试下载镜像
        logger.info(f"Downloading Windows image from {download_url}")
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(kvm_base):
                ...
                if size > 100 * 1024 * 1024:  # 大于100MB
                    logger.info(f"Downloaded Windows image: {size // (1024*1024)}MB")
                    return True
                else:
    ...
    def _build_windows_with_virtbuilder(self, image, kvm_base):
        ...
        logger.info(f"Trying virt-builder for {image}")
        ...
        try:
            ...
            logger.info(f"Running virt-builder for {os_variant}")
            result = safe_subprocess_run(cmd, timeout=1800)

            if result.returncode == 0 and os.path.exists(kvm_base):
                size = os.path.getsize(kvm_base) // (1024*1024)
                logger.info(f"Built Windows image with virt-builder: {size}MB")
                return True
            else:
                logger.error(f"virt-builder failed: {result.stderr}")
        except Exception as e:
            logger.error(f"virt-builder error: {e}")
    ...
    def _handle_get_console(self, name):
        ...
        try:
            ...
            logger.warning(f"Invalid name for console: {e}")
            self._send_json({"error": str(e)}, 400)
        except Exception as e:
            logger.error(f"Error getting console for {name}: {e}")
            self._send_json({"error": str(e)}, 500)
    ...
    def _handle_get_traffic(self, machine_id):
        ...
        try:
            # 验证machine_id格式
            if not re.match(r'^[a-zA-Z0-9_-]+$', str(machine_id)):
                self._send_json({"error": "invalid machine_id format"}, 400)
                return
            ...
            logger.warning(f"Invalid name for traffic: {e}")
            self._send_json({"error": str(e), "bandwidth_mbps": 0})
        except Exception as e:
            logger.error(f"Error getting traffic for {machine_id}: {e}")
            self._send_json({"error": str(e), "bandwidth_mbps": 0})
    ...
    def _handle_opengfw_status(self):
        ...
        try:
            ...
            result = safe_subprocess_run(["pgrep", "-f", "opengfw"], timeout=5)
            opengfw_running = result.returncode == 0
            config_exists = os.path.exists("/etc/opengfw/config.yaml")
            nft_result = safe_subprocess_run(["nft", "list", "table", "opengfw"], timeout=5)
            ...
            logger.error(f"Error getting OpenGFW status: {e}")
            self._send_json({"error": str(e), "installed": False, "running": False})

    def _handle_opengfw_install(self):
        """Install OpenGFW"""
        try:
            safe_subprocess_run(["apt-get", "update", "-qq"], timeout=LONG_TIMEOUT)
            safe_subprocess_run([
                "apt-get", "install", "-y", "-qq",
            ...
            safe_subprocess_run(["rm", "-rf", work_dir], timeout=10)
            safe_subprocess_run(
            ...
            if build_result.returncode != 0:
                logger.error(f"OpenGFW build failed: {build_result.stderr}")
                self._send_json({"status": "error", "error": build_result.stderr})
                return

            safe_subprocess_run(["chmod", "+x", "/usr/local/bin/opengfw"], timeout=5)
            os.makedirs("/etc/opengfw", exist_ok=True)
            safe_subprocess_run(["systemctl", "enable", "nftables"], timeout=10)
            safe_subprocess_run(["systemctl", "start", "nftables"], timeout=10)

            logger.info("OpenGFW installed successfully")
            self._send_json({"status": "installed"})
        except subprocess.TimeoutExpired:
            logger.error("OpenGFW install timeout")
            self._send_json({"status": "error", "error": "operation timeout"})
        except Exception as e:
            logger.error(f"OpenGFW install error: {e}")
    ...
    def _handle_opengfw_config(self):
        ...
        try:
            ...
            safe_subprocess_run(["pkill", "-f", "opengfw"], timeout=5)
            subprocess.Popen(
                ["/usr/local/bin/opengfw", "-c", "/etc/opengfw/config.yaml"],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )

            logger.info(f"OpenGFW configured with {len(rules)} rules")
            self._send_json({"status": "configured", "rules_count": len(rules)})
        except urllib.request.URLError as e:
            logger.error(f"Failed to fetch OpenGFW config: {e}")
            self._send_json({"status": "error", "error": str(e)})
        except Exception as e:
            logger.error(f"OpenGFW config error: {e}")
            self._send_json({"status": "error", "error": str(e)})
...
'''
            # 使用临时文件来避免shell注入
            temp_file = "/tmp/nft_script_temp"
            with open(temp_file, "w") as f:
                f.write(nft_script)
            safe_subprocess_run(["nft", "-f", temp_file], timeout=10)
            os.remove(temp_file)
    ...
    def _handle_opengfw_uninstall(self):
        ...
        try:
            safe_subprocess_run(["pkill", "-f", "opengfw"], timeout=5)
            safe_subprocess_run(["rm", "-f", "/usr/local/bin/opengfw"], timeout=5)
            safe_subprocess_run(["rm", "-rf", "/etc/opengfw"], timeout=5)
            safe_subprocess_run(["nft", "delete", "table", "ip", "opengfw"], timeout=5)
            logger.info("OpenGFW uninstalled successfully")
            self._send_json({"status": "uninstalled"})
        except Exception as e:
            logger.error(f"OpenGFW uninstall error: {e}")
            self._send_json({"status": "error", "error": str(e)})

    def do_POST(self):
        if not self._check_auth():
            self._send_json({"error": "unauthorized"}, 401)
            logger.warning("Unauthorized POST request")
            return
        ...
        try:
            ...
            logger.error(f"JSON decode error in POST: {e}")
            self._send_json({"error": "invalid JSON body"}, 400)
            return
        except ValueError as e:
            logger.error(f"Invalid content length in POST: {e}")
        ...
        try:
            ...
            logger.warning(f"Validation error in POST {path}: {e}")
            self._send_json({"error": str(e)}, 400)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout in POST {path}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
    ...
    def _handle_create(self, body):
        ...
        try:
            ...
            # 验证并限制参数范围
            cpu = max(1, min(int(body.get("cpu", 1)), 64))  # 限制1-64
            memory = max(512, min(int(body.get("memory", 1024)), 32768))  # 限制512MB-32GB
            disk = max(1, min(int(body.get("disk", 10)), 500))  # 限制1-500GB
            ...
            # 验证image格式
            if not re.match(r'^[a-zA-Z0-9]+:[a-zA-Z0-9\.]+$', image):
                self._send_json({"error": "invalid image format"}, 400)
                return
            ...
            if virt == "lxd":
                ...
                result = safe_subprocess_run(cmd, timeout=LONG_TIMEOUT)
                ...
                if result.returncode == 0:
                    # Set root password safely using chpasswd
                    # 使用更安全的方式设置密码
                    safe_subprocess_run([
                        "lxc", "exec", name, "--",
                        "bash", "-c", f"echo 'root:{escape_shell_arg(root_password)}' | chpasswd"
                    ], timeout=30)
                    ...
                    if app_image and APP_IMAGES.get(app_image, {}).get("docker_image"):
                        ...
                        if app_config:
                            ...
                            safe_subprocess_run(docker_cmd, timeout=LONG_TIMEOUT)

                    # Get IP
                    ip_result = safe_subprocess_run(
                    ...
                    logger.error(f"Failed to create LXD container {name}: {result.stderr}")
                    self._send_json({
                        "status": "error",
                        "error": result.stderr
                    })
                ...
                # Windows 镜像自动准备
                if is_windows:
                    logger.info(f"Preparing Windows image for {image}...")
                    ...
                    if lxd_image:
                        # 创建临时容器导出镜像
                        safe_subprocess_run(["lxc", "launch", lxd_image, "temp-base", "--ephemeral"], timeout=LONG_TIMEOUT)
                        time.sleep(5)
                        safe_subprocess_run([
                            "lxc", "exec", "temp-base", "--",
                            "dd", "if=/dev/sda", "of=/tmp/rootfs.img"
                        ], timeout=LONG_TIMEOUT)
                        safe_subprocess_run(["lxc", "file", "pull", "temp-base/tmp/rootfs.img", kvm_base], timeout=LONG_TIMEOUT)
                        safe_subprocess_run(["lxc", "delete", "--force", "temp-base"], timeout=10)
                        safe_subprocess_run(["qemu-img", "convert", "-O", "qcow2", kvm_base + ".img", kvm_base], timeout=30)
                        if os.path.exists(kvm_base):
                            logger.info(f"Created base image: {kvm_base}")
                        else:
                            self._send_json({
                                "status": "error",
                                "error": "Failed to create Linux base image"
                            })
                            return
                    else:
                        self._send_json({
                ...
                if result.returncode == 0:
                    ...
                    logger.info(f"Created KVM VM: {name}")
                    self._send_json(response_data)
                else:
                    # 释放已分配的端口
                    ...
                    logger.error(f"Failed to create KVM VM {name}: {result.stderr}")
                    self._send_json({
                        "status": "error",
                        "error": result.stderr
                    })
            ...
            logger.error(f"Timeout creating VM: {e}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
    ...
    def _get_vm_ip(self, name, timeout=60):
        ...
        try:
            ...
            logger.warning(f"Failed to get VM IP for {name}: {e}")
            return None
    ...
    def _setup_novnc_port(self, name, vnc_port, web_port):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"noVNC started on port {web_port}")
                return web_port
            else:
                logger.warning(f"Failed to start novnc: {result.stderr}")
                release_port(web_port)
                return 0
        except Exception as e:
            logger.warning(f"novnc setup failed: {e}")
            release_port(web_port)
            return 0
    ...
    def _handle_stop(self, name):
        ...
        try:
            ...
            if virt == "lxd":
                safe_subprocess_run(["lxc", "stop", name], timeout=30)
                cmd = ["lxc", "delete", "--force", name]
            else:
                safe_subprocess_run(["virsh", "destroy", name], timeout=30)
                cmd = ["virsh", "undefine", name, "--nvram"]

            result = safe_subprocess_run(cmd, timeout=30)

            # Also stop any associated novnc container
            safe_subprocess_run(["docker", "rm", "-f", f"novnc-{name}"], timeout=5)

            if result.returncode == 0:
                logger.info(f"Stopped and deleted: {name}")
                self._send_json({
                    "status": "stopped",
                    "output": result.stdout,
                })
    ...
    def _handle_reinstall(self, name, body):
        ...
        try:
            ...
            logger.error(f"Failed to fetch machine info for reinstall: {e}")
            self._send_json({"status": "error", "error": str(e)})
        except Exception as e:
            logger.error(f"Reinstall error for {name}: {e}")
            self._send_json({"status": "error", "error": str(e)})
    ...
    def _handle_exec(self, name, body):
        ...
        try:
            ...
            if virt == "lxd":
                ...
                # KVM - use qemu-guest-agent
                result = safe_subprocess_run(
                    ["virsh", "qemu-agent-command", name, f'{{"execute":"guest-exec", "arguments":{"path":"/bin/bash", "arg":["-c", "{command}"]}}}'],
                    timeout=LONG_TIMEOUT
                )

            logger.info(f"Executed command in {name}")
            self._send_json({
            ...
            logger.warning(f"Validation error in exec: {e}")
            self._send_json({"error": str(e)}, 400)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout executing command in {name}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
            logger.error(f"Exec error for {name}: {e}")
            self._send_json({"error": str(e)}, 500)
    ...
    def _handle_app_install(self, name, body):
        ...
        try:
            ...
            # 验证app_image名称
            if not re.match(r'^[a-zA-Z0-9_-]+$', app_image):
                self._send_json({"error": "invalid app_image format"}, 400)
                return
            ...
            if virt == "lxd":
                ...
                result = safe_subprocess_run(docker_cmd, timeout=LONG_TIMEOUT)

                logger.info(f"App {app_image} installed in {name}")
                self._send_json({
            ...
            logger.warning(f"Validation error in app install: {e}")
            self._send_json({"error": str(e)}, 400)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout installing app in {name}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
            logger.error(f"App install error for {name}: {e}")
            self._send_json({"error": str(e)}, 500)
    ...
    def _handle_app_uninstall(self, name, body):
        ...
        try:
            ...
            if virt == "lxd":
                ...
                logger.info(f"App {app_image} uninstalled from {name}")
                self._send_json({
            ...
            logger.warning(f"Validation error in app uninstall: {e}")
            self._send_json({"error": str(e)}, 400)
        except subprocess.TimeoutExpired:
            logger.error(f"Timeout uninstalling app from {name}")
            self._send_json({"error": "operation timeout"}, 504)
        except Exception as e:
            logger.error(f"App uninstall error for {name}: {e}")
            self._send_json({"error": str(e)}, 500)

    def do_DELETE(self):
        if not self._check_auth():
            self._send_json({"error": "unauthorized"}, 401)
            logger.warning("Unauthorized DELETE request")
            return
    ...
    while True:
        try:
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                try:
                    info_result = safe_subprocess_run(
                    ...
                    logger.warning(f"Stats error for {machine_name}: {e}")
        except Exception as e:
            logger.warning(f"Stats loop error: {e}")
...
def detect_hardware():
    ...
    try:
        result = safe_subprocess_run(["nproc", "--all"], timeout=DEFAULT_TIMEOUT)
    ...
    try:
        result = safe_subprocess_run(["grep", "MemTotal", "/proc/meminfo"], timeout=DEFAULT_TIMEOUT)
        if result.returncode == 0 and result.stdout.strip():
    ...
    try:
        result = safe_subprocess_run(["df", "-BG", "/"], timeout=DEFAULT_TIMEOUT)
        if result.returncode == 0 and result.stdout.strip():
        ...
        logger.warning(f"Failed to detect disk: {e}")
...
if __name__ == "__main__":
    ...
    logger.info(f"Agent running on port 19527, virt_type={VIRT_TYPE}")
